...
- [ ] Reminder behavior passes.
- [ ] Session timing passes soak testing.
- [ ] Soundscape claims match implementation.
- [ ] Privacy/backup behavior matches policy.
- [ ] TalkBack and 200% font flows pass.
- [ ] Play assets and policy URL are final.
- [ ] Pre-launch findings are resolved or explicitly accepted.

---

# 12. Recommended pull-request sequence

Keep changes reviewable and avoid combining visual redesign with data migrations.

| PR | Scope | Depends on |
|---:|---|---|
| 1 | Remove custom debug signing; pin JDK/toolchain; fix README target mismatch | None |
| 2 | Add CI build/lint/test skeleton | PR 1 |
| 3 | Remove fabricated Home/Progress values; add truthful empty states | PR 1 |
| 4 | Exact-second session model and aggregation tests | PR 2 |
| 5 | Room schema export and tested migrations | PR 4 |
| 6 | Monotonic session engine and lifecycle policy | PR 4 |
| 7 | Reminder time UI, scheduling, cancellation, notification destination | PR 2 |
| 8 | Navigation Compose and correct back stacks | PR 2 |
| 9 | ViewModels, lifecycle-aware state, application container | PR 8 |
| 10 | Design tokens and core component states | PR 3, PR 9 |
| 11 | Home simplification and onboarding personalization | PR 10 |
| 12 | Favorites, Explore lazy list, saved filter/scroll state | PR 9, PR 10 |
| 13 | Detail/custom rhythm consistency and safe validation | PR 5, PR 10 |
| 14 | Soundscape integration and audio focus | PR 6, PR 10 |
| 15 | Adaptive layouts and orientation unlock | PR 10 |
| 16 | Strings, plurals, TalkBack semantics, reduced motion | PR 10, PR 15 |
| 17 | Complete unit, migration, Compose, screenshot, and accessibility tests | All feature PRs |
| 18 | Dependency/security update, release hardening, Play internal build | PR 17 |
...
This table ensures every audit finding has an implementation destination.

| Audit finding | Roadmap owner |
|---|---|
| Fabricated Home/Progress statistics | M1.1, M6.2, M6.8 |
| Destructive Room fallback | M1.3, M10.2 |
| Reminder toggle does not schedule work | M3.1 |
| Reminder time is displayed but not honored | M3.1 |
| Session seconds rounded up to minutes | M1.2 |
| `durationSecondsActual` not populated | M1.2 |
| Missing debug keystore blocks fresh build | M0.1 |
| Home is visually overloaded | M5.2, M6.2 |
| Onboarding goals do not drive Home default | M6.1, M6.2 |
| Favorites cannot be browsed | M6.3 |
| Soundscape assets/controller are unused | M3.2 |
| Sound icon represents guidance cues inaccurately | M3.2, M6.6 |
| Portrait lock and fixed 300dp session orb | M7.1, M6.6 |
| Continuous animations ignore reduced motion in places | M7.4, M8.3 |
| Missing touch feedback and undersized targets | M5.3, M7.2 |
| Colors, shapes, typography, and spacing are scattered | M5.1 |
| Missing first-use empty states | M1.1, M6.2, M6.8 |
| Manual non-restorable navigation | M4.1 |
| Legal pages return to the wrong origin | M4.1 |
| Business logic is concentrated in one composable | M4.2, M4.3 |
| Flows are not lifecycle-aware | M4.2 |
| Frame-based session timer can drift | M2.1 |
| Mid-session rhythm switch saves original rhythm | M2.3 |
| Backup setting conflicts with local-only privacy claim | M9.1 |
| 237 hard-coded UI strings | M7.3 |
| No unit or instrumented tests | M10.1–M10.5 |
| No fresh clone release evidence | M0.1–M0.4, M11 |
...
|---|---|---|
| Unknown previously released Room schemas | User data loss | Establish release history before changing schema; obtain old APK/schema fixtures |
| Large architecture rewrite stalls product work | Long unstable branch | Land navigation and ViewModels in small behavior-preserving PRs |
| UI redesign hides correctness regressions | False confidence | Complete P0 data/timer work before visual overhaul |
| Ambient audio increases APK size/battery use | Poor installs/session quality | Measure size, normalize/compress assets, manage lifecycle/audio focus |
| Exact reminder expectations conflict with Doze | User disappointment | Describe reminders as approximate; avoid exact-alarm permission |
| Removing portrait lock exposes layout defects | Broken tablets/landscape | Add adaptive layouts and screenshot tests before unlocking in production |
| Localization causes layout expansion | Clipping | Remove fixed text heights and test long pseudo-localized strings |
| Strong local-only claims conflict with platform backup | Policy violation/trust loss | Disable backup or rewrite every public claim consistently |
| Dependency upgrades break Compose/KSP compatibility | Build instability | Upgrade as a tested version set in a dedicated PR |
| No analytics limits rollout insight | Slow issue detection | Use Play vitals/pre-launch plus opt-in support consistent with privacy policy |

---

# 15. Suggested delivery milestones

These are sequencing targets, not fixed calendar promises. Re-estimate after Milestone 0 based on one developer versus a team.

## Release A — Trustworthy internal build
...
- reliable timer;
- working reminders;
- accurately labeled media controls.

## Release B — Production UX foundation

Includes Milestones 4–7:

- restorable navigation;
- ViewModels and lifecycle state;
...
## Release C — Public launch candidate

Includes Milestones 8–11:
...
Start with these tasks and do not begin the large UI redesign until they are merged:

1. Remove the custom debug signing dependency and prove a clean-clone build.
2. Add CI.
3. Remove fabricated activity and add first-use empty states.
4. Define exact session-duration display and storage rules.
5. Establish Room schema history and write migrations.
6. Replace the frame-counted timer with a monotonic session engine.
7. Wire reminder scheduling and time selection.
8. Decide and enforce Android backup/privacy behavior.

Once those foundations are stable, begin Navigation/ViewModel work and the screen-by-screen UI program.

## Final product target

The finished Lumyrinth should feel calm and premium, but more importantly it should be honest, durable, accessible, and predictable. A mass-use wellness app earns trust through accurate progress, safe persistence, reliable sessions, restrained design, and evidence-backed releases. The cosmic visual identity should remain the emotional signature; the roadmap above makes the engineering underneath worthy of that presentation.
